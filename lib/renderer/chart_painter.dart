...
class ChartPainter extends BaseChartPainter {
  ...
  void initChartRenderer() {
    ...
    if (mVolRect != null) {
      mVolRenderer = VolRenderer(mVolRect!, mVolMaxValue, mVolMinValue, mChildPadding, priceFormat, this.chartStyle, this.chartColors);
    }
  ...
  void drawDate(Canvas canvas, Size size) {
    ...
    for (var i = 0; i <= mGridColumns; ++i) {
      ...
      if (translateX >= startX && translateX <= stopX) {
        ...
        TextPainter tp = getTextPainter(getDate(datas![index].time, false), null);
...
//    }
  ...
  void drawCrossLineText(Canvas canvas, Size size) {
    ...
    TextPainter tp = getTextPainter(priceFormat(point.close), chartColors.crossTextColor);
    ...
    TextPainter dateTp = getTextPainter(getDate(point.time, true), chartColors.crossTextColor);
  ...
  void drawMaxAndMin(Canvas canvas) {
    ...
    if (x < mWidth / 2) {
      //draw right
      TextPainter tp = getTextPainter("── " + priceFormat(mMainLowMinValue), chartColors.minColor);
    ...
    if (x < mWidth / 2) {
      //draw right
      TextPainter tp = getTextPainter("── " + priceFormat(mMainHighMaxValue), chartColors.maxColor);
      tp.paint(canvas, Offset(x, y - tp.height / 2));
    } else {
      TextPainter tp = getTextPainter(priceFormat(mMainHighMaxValue) + " ──", chartColors.maxColor);
      tp.paint(canvas, Offset(x - tp.width, y - tp.height / 2));
  ...
  String getDate(int? date, bool isInfoWindow) => dateFormat(
        DateTime.fromMillisecondsSinceEpoch(date ?? DateTime.now().millisecondsSinceEpoch),
    isInfoWindow? infoMFormat :mFormats,
      );
